              ...
              <tbody>
                ...
                @foreach($exams as $exam)
                  <tr>
                    ...
                    @if($count == 0)
                      @if($questionCount > 0)
                        <td><a href="{{route('viewQuestionsForUser', [$sub_module_slug,$exam->slug])}}" class="btn btn-primary">Take Test</a></td>
                      @else                        
                        <td>Question Not Available</td>
                      @endif
                    @else
                      <td><a href="{{route('viewQuestionsForUser', [$sub_module_slug,$exam->slug])}}" class="btn btn-primary">Take Test</a> <a href="{{ route('getSubModuleResult',[$main_module_slug,$sub_module_slug]) }}">(Result Link)</a></td>
                    @endif
                  </tr>
                  @php $key++; @endphp
                @endforeach
