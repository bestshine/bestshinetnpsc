...
app.controller("feeTable",['$scope','$http',function($scope,$http){
	...
	$scope.fees=[
        ...
        $scope.getfee = function(){
            ...
            if(session == "" || classId == undefined){
                $("#jsErrors").html("Please select class & Session");
            }else{
                window.location.href = 'https://shineschoolapp.com/users/fee/structure/' + session + '/'+classId+'';
            }
        };
    ...
    $scope.submitInstallment = function($scope){
        ...
        if(totalRowCount == installmentArrayLength){
        	$(".show-error-msg").show();
	       	$http({
	            method: 'POST',
	            url: 'https://shineschoolapp.com/users/installment',
...
app.controller("feeSlipTable",['$scope','$http',function($scope,$http){
	$scope.deleteRow=function(){
		...
		$http({
                method: 'DELETE',
                url: 'https://shineschoolapp.com/fee/slip/'+editButtonId[1]+'',
                dataType: "html"
            }).then(function successCallback(response) {
				console.log(response);
...
app.controller("payment",['$scope','$http',function($scope,$http){
	$scope.getStudent = function(){
		if($scope.registerNo){
			$http({
                method: 'POST',
                url: 'https://shineschoolapp.com/users/payment/student/'+$scope.registerNo+'',
                dataType: "html"
            }).then(function successCallback(response) {
